...
def compare_title_h1(title: Optional[str], h1: Optional[str]) -> tuple[Optional[bool], Optional[float]]:
...
def extract_first_200_words(soup: BeautifulSoup) -> Optional[str]:
    ...
    for element in body.descendants:
        if not isinstance(element, NavigableString):
            continue
        if any(
            getattr(p, "name", None) in excluded
...
_UTILITY_PATH_PREFIXES = (
    "/robots.txt", "/sitemap", "/cdn-cgi/", "/wp-admin/", "/wp-login",
    "/login", "/logout", "/sign-in", "/sign-out", "/auth/",
    "/.well-known/", "/api/", "/_next/", "/__", "/static/", "/assets/",
    "/favicon", "/manifest.",
)

_UTILITY_PATH_EXTENSIONS = (
    ".xml", ".txt", ".json", ".pdf", ".png", ".jpg", ".jpeg",
