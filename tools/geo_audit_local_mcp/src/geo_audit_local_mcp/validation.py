...
def validate_claims(
    ...
    for c in contradictions:
        downgrades.append(ConfidenceDowngrade(
            claim=c.detail,
...
def _check_artifact_contradictions(
    ...
    for da in evidence.domain_artifacts:
        ...
        findings.extend(_check_single_artifact(
            draft_lower, domain, "robots.txt", da.robots_txt.status,
        ))
        findings.extend(_check_single_artifact(
            draft_lower, domain, "sitemap.xml", da.sitemap_xml.status,
        ))
        findings.extend(_check_single_artifact(
            draft_lower, domain, "llms.txt", da.llms_txt.status,
        ))
        findings.extend(_check_single_artifact(
            draft_lower, domain, "llms-full.txt", da.llms_full_txt.status,
        ))
...
def _check_single_artifact(
    ...
    draft_claims_not_found = any(
        re.search(p, draft_lower) for p in not_found_patterns
    )
    draft_claims_found = any(
        re.search(p, draft_lower) for p in found_patterns
    )
...
def _check_missing_high_signal_facts(
    ...
    for da in evidence.domain_artifacts:
        ...
        if da.llms_full_txt.status == ArtifactStatus.FOUND and not _LLMS_FULL_TXT_RE.search(draft_lower):
            findings.append(ValidationFinding(
                severity=Severity.MISSING_FACT,
                category="missing_artifact_mention",
        ...
        if da.declared_sitemaps:
            for sitemap_url in da.declared_sitemaps:
                if "/sitemap.xml" not in sitemap_url:
                    ...
                    if base_path.lower() not in draft_lower and "declared sitemap" not in draft_lower:
                        findings.append(ValidationFinding(
                            severity=Severity.MISSING_FACT,
