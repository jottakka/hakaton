...
async def run_aioa_benchmark(
    ...
    if raw_prompts:
        prompts = [
            Prompt(id=p.get("id", f"p{i}"), text=p["text"]) for i, p in enumerate(raw_prompts)
        ]
    else:
        prompts = [
            Prompt(
    ...
    if raw_terms:
        terms = [
            SearchTerm(id=t.get("id", f"s{i}"), query=t["query"]) for i, t in enumerate(raw_terms)
        ]
    else:
        terms = [
            SearchTerm(
    ...
    artifacts: list[str] = [
        str(p.relative_to(output_dir)) for p in run_output_dir.rglob("*") if p.is_file()
    ]
