...
- This workspace includes a local `geo-site-audit` workflow backed by deterministic GEO audit tools (`CollectGeoEvidence` and `ValidateGeoAuditClaims`) in `tools/geo_audit_arcade/` (Arcade MCPApp) and `tools/geo_audit_local_mcp/` (FastMCP).
